...
{   
	...
	for(var i=0; i< node.attributes.length; i++)
	{
	    ...
	    modalFormQuery.find('input[name='+att.nodeName.toLowerCase()+']').val(att.nodeValue);
	}
	...
	var attListQuery = tipQuery.children('.ui-widget-header').text(domNode.tagName).end().find('dl').empty().css('display', 'none');
	var attributes = domNode.attributes;
	if(attributes.length)
	    attListQuery.css('display', 'block');
	// Build list of Attributes	    
	for(var i=0; i<attributes.length; i++)	   
	{
	    var att = attributes.item(i);
	    attListQuery.append('<dt>'+att.nodeName+':</dt><dd>'+att.nodeValue+'</dd>');
	}
	// Position and Show
	...
	{	
		...
		for(var i in tei.element)
		{
		    ...
		    {
			var opt = addSelection(i.toUpperCase(), '', insrtSubMenuQuery);
	...
	if(!srcIconQuery.data('active')()) // Content area is not a <textarea>	
	{
	    ...
	    var remButton = targ.parents().andSelf().is('#'+remIconId);
	    var remEnab = remIconQuery.data('enabled')();
	    var edButton = targ.parents().andSelf().is('#'+editIconId);
	    var edEnab = editIconQuery.data('enabled')();
	    var chooseOption = (remButton && remEnab) || (edButton && edEnab);
